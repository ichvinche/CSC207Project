...
public class BoggleGame {
    ...
    private void findAllWords(Map<String,ArrayList<Position>> allWords, Dictionary boggleDict, BoggleGrid boggleGrid) {
        for (int i = 0; i < boggleGrid.numRows(); i++) {
            for (int j = 0; j < boggleGrid.numRows(); j++) {
                findWordsHelper(allWords, boggleDict, boggleGrid, new ArrayList<Position>(), i, j, "");
            }
        }
    }

    /*
     * Helper method for findAllWords that returns a Map containing all of the words found within the boggleGrid grid.
     *
     * @param allWords HashMap of all the words in th grid with its positions
     * @param boggleDict Dictionary of legal words
     * @param boggleGrid A boggle grid with a letter at each position of the grid
     * @param positions List of positions for the current word being searched for
     * @param row The row position of the letter we are starting the word with
     * @param col The column position of the letter we are starting the word with
     * @param s The word string we are creating with the letters of the boggleGrid grid
     */
    private void findWordsHelper(Map<String,ArrayList<Position>> allWords, Dictionary boggleDict,
                                 BoggleGrid boggleGrid, ArrayList<Position> positions, int row, int col, String s) {
        Position currPos = new Position(row, col);
        char firstChar = boggleGrid.getCharAt(row, col);
        Position[][] adjacentPositions = positionsArray(row, col);
        ...
        for (int i = 0; i < adjacentPositions.length; i++) {
            for (int j = 0; j < adjacentPositions.length; j++) {
                if (adjacentPositions[i][j].getRow(i) < 0 || adjacentPositions[i][j].getCol(j) < 0 ||
                        adjacentPositions[i][j].getRow(i) >= boggleGrid.numRows() ||
                        adjacentPositions[i][j].getCol(j) >= boggleGrid.numCols()) {
                    continue;
                } else {
                    int currRow = adjacentPositions[i][j].getRow(i);
                    int currCol = adjacentPositions[i][j].getCol(j);

                    char currChar = boggleGrid.getCharAt(currRow, currCol);

                    if (!containsPosition(adjacentPositions[i][j], positions)) {
                        s += currChar;

                        if (boggleDict.isPrefix(s)) {
                            if (s.length() >= 4 && boggleDict.containsWord(s)) {
                                allWords.put(s, positions);
                                s = s.substring(0, s.length() - 1);
                                findWordsHelper(allWords, boggleDict, boggleGrid, positions, currRow, currCol, s);
                            } else {
                                s = s.substring(0, s.length() - 1);
                                findWordsHelper(allWords, boggleDict, boggleGrid, positions, currRow, currCol, s);
                            }
                        } else if (s.length() >= 4 && boggleDict.containsWord(s)) {
                            allWords.put(s, positions);
                        } else {
                            s = s.substring(0, s.length() - 1);
                        }
    ...
    /*
     * Helper method that returns a 2-dimensional array consisting of the row and column position values of the
     * adjacent positions given we start at some character c at position (row, col).
     *
     * @param row The row value of the letter we are searching for the adjacent letter positions
     * @param col The column value of the letter we are searching for the adjacent letter positions
     */
    private Position[][] positionsArray(int row, int col) {
        Position[][] adjacentPositions = new Position[3][3];
        for (int i = 0; i <= 2; i++) {
            for (int j = 0; j <= 2; j++) {
                adjacentPositions[i][j] = new Position(row + i - 1, col + j - 1);
            }
        }
    ...
    /*
     * Helper method that checks whether a given position has the same values as another position in the array list
     *
     * @param pos The position we are searching for in the list
     * @param arrayPos The list of given positions
     */
    private boolean containsPosition(Position pos, ArrayList<Position> arrayPos) {
        for (Position p: arrayPos) {
            if (p.getRow(0) == pos.getRow(0) && p.getCol(0) == pos.getCol(0)) {
                return true;
            }
        }
        return false;
